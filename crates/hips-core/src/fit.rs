...
impl Gravity {
    ...
    fn smart(self) -> Option<Interest> {
        match self {
            ...
            Gravity::Auto | Gravity::Face => Some(Interest::Attention),
            _ => None,
        }
    }
}
...
pub fn plan(
    ...
    match (fit, width, height) {
        ...
        (FitMode::Cover, Some(w), Some(h)) => cover(src_w, src_h, w, h, gravity, true),
        (FitMode::Crop, Some(w), Some(h)) => crop(src_w, src_h, w, h, gravity),
...
fn cover(src_w: u32, src_h: u32, w: u32, h: u32, gravity: Gravity, allow_upscale: bool) -> Plan {
    ...
    directional(src_w, src_h, w, h, scale, gravity)
}

fn crop(src_w: u32, src_h: u32, w: u32, h: u32, gravity: Gravity) -> Plan {
    ...
    directional(src_w, src_h, w, h, fill, gravity)
}

fn directional(src_w: u32, src_h: u32, w: u32, h: u32, scale: f64, gravity: Gravity) -> Plan {
    ...
    let x = match gravity {
        ...
        Gravity::East | Gravity::NorthEast | Gravity::SouthEast => resize_w - w,
        _ => (resize_w - w) / 2,
    };
    let y = match gravity {
        Gravity::North | Gravity::NorthWest | Gravity::NorthEast => 0,
        Gravity::South | Gravity::SouthWest | Gravity::SouthEast => resize_h - h,
        _ => (resize_h - h) / 2,
    };
...
mod tests {
    ...
    fn cover_missing_dimension_degrades_to_fit() {
        let p = plan(800, 600, Some(300), None, FitMode::Cover, Gravity::Center);
        assert_eq!(
            p,
            Plan::Fit {
                w: Some(300),
