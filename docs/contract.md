...
Only meaningful for `cover`/`crop`. `center`/`auto`/`face` use libvips smartcrop (`auto` and
`face` map to attention-based smartcrop — there is no face-detection model, attention is the
approximation). Directional values crop a window anchored to that edge/corner.
